...
if ( $order->get_status() == 'ywraq-new' ) {
	...
	if ( catalog_mode_plugin_enabled() ) {
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$hide_price = apply_filters( 'yith_ywraq_hide_price_template', WC()->cart->get_product_subtotal( $_product, $item['qty'] ), $_product->id, $item);
			if ( $hide_price == '' ) {
				$show_price = false;
			}
		}
...
$print_button_pdf = get_option( 'ywraq_pdf_attachment' ) == 'yes' && get_option( 'ywraq_pdf_in_myaccount' ) == 'yes' && $pdf_file;
